...
const getCafes = async (location) => {
  ...
  const cafes = await Cafe.findAll({
    ...
    attributes: {
      include: [
        [Sequelize.fn('COUNT', Sequelize.col('EmployeeCafes.cafe_id')), 'employeeCount'],
      ],
    },
    group: ['Cafe.id'],
    order: [[Sequelize.fn('COUNT', Sequelize.col('EmployeeCafes.cafe_id')), 'DESC']],
  });
...
const createNewCafe = async ({ name, description, logo, location }) => {
  try {
    ...
    if (!name || !description || !location) {
      const error = new Error('Missing required fields: name, description, or location.');
      error.status = 400; 
      throw error;
    }
    ...
    const maxLength = 255;
    if (name.length > maxLength || description.length > maxLength || logo.length > maxLength || location.length > maxLength) {
      const error = new Error(`Fields cannot exceed ${maxLength} characters.`);
      error.status = 400; 
      throw error;
    }
...
module.exports = { getCafeById, getCafes, createNewCafe, updateCurrentCafe, deleteCafeById };